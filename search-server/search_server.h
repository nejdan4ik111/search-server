...
private:
    ...
    Query ParseQueryPar(std::string_view text, bool remove_duplicates) const;
...
std::vector<Document> SearchServer::FindTopDocuments(ExecutionPolicy&& policy, std::string_view raw_query, DocumentPredicate document_predicate) const {
    ...
    std::vector<Document> result_documents(std::min(matched_documents.size(), static_cast<std::vector<Document>::size_type>(MAX_RESULT_DOCUMENT_COUNT)));
    std::partial_sort_copy(policy,
        matched_documents.begin(), matched_documents.end(),
        result_documents.begin(), result_documents.end(),
        [](const Document& lhs, const Document& rhs) {
            if (std::abs(lhs.relevance - rhs.relevance) < EPSILON) {
                return lhs.rating > rhs.rating;
            }
