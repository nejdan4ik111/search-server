...
std::tuple<std::vector<std::string_view>, DocumentStatus> SearchServer::MatchDocument(const std::execution::parallel_policy& policy, std::string_view raw_query, int document_id) const {
    ...
    const Query& query = ParseQueryPar(raw_query,true);
...
SearchServer::Query SearchServer::ParseQuery(std::string_view text) const {
    ...
    result.minus_words.resize(std::unique(result.minus_words.begin(), result.minus_words.end()) - result.minus_words.begin());
    result.plus_words.resize(std::unique(result.plus_words.begin(), result.plus_words.end()) - result.plus_words.begin());

    return result;
}

SearchServer::Query SearchServer::ParseQueryPar(const std::string_view text, bool remove_duplicates) const {
